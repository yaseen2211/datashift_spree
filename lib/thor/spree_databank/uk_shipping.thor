          ...
          @cat_rates_matrix.each do |cat, rates|
            ...
            @shipping_methods = []
            {
              ...
              ship_method_attributes = {
                :name => name,
                :zones => [Spree::Zone.find_by_name(zone)],
                :shipping_categories => [current_shipping_category],
                :calculator_type => options[:calc]
              }

              # Without protection removed from Rails 4
              @shipping_methods << Spree::ShippingMethod.create!( ship_method_attributes )
